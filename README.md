...
**주요 업데이트:**
- **2025-11-11**: 예약 타입별 워크플로우 분리 및 에러 처리 개선
  - Switch 노드로 Table/Room 예약 경로 분리
  - Table 예약: 영수증 처리 없이 바로 예약번호 생성 및 Notion 저장
  - Room 예약: Base64 변환 → Google Drive 업로드 → 예약번호 생성 → Notion 저장
  - 프론트엔드 에러 처리 개선 (HTTP 200이지만 status='error'인 경우 처리)
  - Notion 저장 노드에서 Deposit Receipt 필드 조건부 처리 (Table 예약 시 제외)
