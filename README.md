...
### 이메일이 없는 예약 — 리마인더는 건너뜁니다
전화·LINE 예약은 직원이 Notion에 직접 입력하며, 이메일 칸이 비어 있거나 요청사항이 적혀 들어오는 경우가 있습니다.
`리마인더 데이터 준비` 노드가 **이메일 형식이 유효한 예약만** 발송 대상으로 삼습니다(2026-08-25 적용).

- 적용 전: 빈 값만 걸렀기 때문에 `ขอชั้น 2` 같은 값이 Gmail로 넘어가 **400 Recipient address required** → 같은 배치의 정상 예약까지 발송이 멈췄습니다(실사고 2026-08-25, 실행 26515).
- 적용 후: 형식이 아닌 값은 조용히 제외되고 나머지는 정상 발송됩니다.
- ⚠️ 부작용: **이메일 칸을 메모로 쓰면 그 손님은 리마인더를 못 받습니다.** 요청사항은 `Special Request` 칸에 적고, 이메일이 없으면 칸을 비워 둡니다.

### 확정·취소 메일은 웹 폼 예약에만 해당
관리자 확정/취소 버튼은 웹 폼 접수 시 나가는 관리자 메일에만 붙습니다(`Webhook → Notion에 저장 → 관리자 이메일 발송` 경로).
직원이 Notion에 직접 입력한 예약은 이 경로를 타지 않습니다. 웹 폼 이메일 칸은 `type="email" required`라 형식이 강제됩니다.

### 인원 상한 (2026-09-08)
입력 상한은 지점·공간에 따라 자동으로 바뀐다. 정본은 `index.html`의 `ROOM_CAPACITY` / `HALL_CAPACITY` 상수 하나이며,
**룸 카드의 "최대 N명" 표시도 같은 상수에서 뽑는다** — 표시와 제한이 어긋날 수 없다.
...
- 홀은 테이블을 붙여 쓸 수 있어 단체 예약이 가능하다(종전 20명 → 50명).
- 공간·지점을 바꿔 상한이 내려가면 이미 입력된 초과 값은 비운다(그대로 두면 제출이 막힌다).
- 서버(n8n `데이터 검증`)에는 인원 상한 검사가 없다 — 제한은 이 프론트 값이 전부.

### 룸 표시명
저장값은 `Room 1`을 유지하고 **표시만** `Private Room`으로 치환합니다(화면 + n8n 이메일 4개 노드).
저장값을 바꾸면 가용성 조회(`Room Number|select equals`)가 과거 예약을 못 찾아 중복 예약이 발생합니다.
